...
//Initialize inodelk domains that will be in use by the transaction.
static int
afr_transaction_inodelk_init (afr_local_t *local, xlator_t *this)
{
        afr_private_t *priv = NULL;
        int           ret   = 0;
        ...
        case AFR_ENTRY_RENAME_TRANSACTION:
                //Compilation complains if new type of transaction is added
                break;
        }
        return ret;
}
...
afr_transaction_local_init (afr_local_t *local, xlator_t *this)
{
        ...
        ret = afr_transaction_inodelk_init (local, this);
        if (ret < 0)
                goto out;
