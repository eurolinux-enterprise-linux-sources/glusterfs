...
static int
afr_set_mdata_domain (xlator_t *this, gf_boolean_t orthogonal_mdata)
{
        int            ret  = 0;
        afr_private_t *priv = this->private;

        if (orthogonal_mdata) {
                ret = gf_asprintf (&priv->mdata_domain, AFR_METADATA_DOMAIN_FMT,
                                   this->name);
        } else {
                ret = gf_asprintf (&priv->mdata_domain, "%s",
                                   this->name);
        }
        if (ret > 0)//return non-zero value only when it is -ve
                ret = 0;
        return ret;
}
...
reconfigure (xlator_t *this, dict_t *options)
{
        ...
        int            index       = -1;
        char          *qtype       = NULL;
        gf_boolean_t   orthogonal_mdata = _gf_false;
	...
	GF_OPTION_RECONF ("post-op-delay-secs", priv->post_op_delay_secs, options,
        ...
        GF_OPTION_RECONF ("ensure-durability", priv->ensure_durability, options,
                          bool, out);
        GF_OPTION_RECONF ("orthogonal-meta-data", orthogonal_mdata, options,
                          bool, out);
        ret = afr_set_mdata_domain (this, orthogonal_mdata);
        if (ret)
                goto out;
...
init (xlator_t *this)
{
	...
	GF_OPTION_INIT ("post-op-delay-secs", priv->post_op_delay_secs, uint32, out);
        ...
        GF_OPTION_INIT ("ensure-durability", priv->ensure_durability, bool,
                        out);
        GF_OPTION_INIT ("orthogonal-meta-data", orthogonal_mdata, bool, out);
        ret = afr_set_mdata_domain (this, orthogonal_mdata);
        if (ret)
                goto out;
        ...
        ret = gf_asprintf (&priv->sh_data_domain, AFR_SH_DATA_DOMAIN_FMT,
                           this->name);
        if (-1 == ret) {
                ret = -ENOMEM;
                goto out;
