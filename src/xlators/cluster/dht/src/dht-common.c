...
preset_layout:
        ...
        gf_msg_debug (this->name, 0,
                      "Creating linkfile on %s(hash) to %s on %s(gfid = %s)",
                  ...
                  int op_errno, gf_dirent_t *orig_entries, dict_t *xdata)
{
        ...
        if (conf->defrag != NULL &&
            (conf->defrag->defrag_status == GF_DEFRAG_STATUS_STARTED ||
            conf->defrag->defrag_status == GF_DEFRAG_STATUS_LAYOUT_FIX_STARTED))
                is_rebalance_running = 1;
        ...
        list_for_each_entry (orig_entry, (&orig_entries->list), list) {
                 ...
                 */
                        ...
                        if (readdir_optimize || is_rebalance_running) {
        ...
        list_for_each_entry (trav, &entries->list, list) {
                ...
                if (!subvol) {
                        gf_log (this->name, GF_LOG_INFO,
                                ...
                                "gfid = %s", lookup_local->loc.path, gfid);
