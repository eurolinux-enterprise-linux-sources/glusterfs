...
/*
 * RHS-2.1u2 and below don't store or restore a volumes op-version and
 * client-op-version unlike upstream glusterfs 3.5 and glusterfs 3.4 (which had
 * the change backported). So, we shouldn't be saving these volinfo fields until
 * we know all peers in the cluster can do the same, ie.
 * cluster op-version >= GD_OP_VERSION_RHS_3_0
 */
int
glusterd_volume_write_op_versions (int fd, glusterd_volinfo_t *volinfo)
{
        int              ret           = -1;
        xlator_t        *this          = NULL;
        glusterd_conf_t *conf          = NULL;
        ...
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR,
                        "Unable to write volume op-version");
                goto out;
        }
        ...
        if (ret)
                gf_log (this->name, GF_LOG_ERROR,
                        "Unable to write volume client-op-version");
out:
        return ret;
}
...
glusterd_volume_exclude_options_write (int fd, glusterd_volinfo_t *volinfo)
{
        ...
        ret = glusterd_volume_write_op_versions (fd, volinfo);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR, "Unable to write volume "
                        "version details for %s", volinfo->volname);
                goto out;
        }
...
glusterd_store_retrieve_snapd (glusterd_volinfo_t *volinfo)
{
         ...
         * to or greater than GD_OP_VERSION_RHS_3_0. The older glusterd would not
