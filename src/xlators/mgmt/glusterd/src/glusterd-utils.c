...
/* Exports a volumes operating versions only if required.
 *
 * RHS-2.1 doesn't export a volume's operating versions. So an RHS-3.0 peer need
 * not export these if the cluster op-version is < GD_OP_VERSION_RHS_3_0
 */
int
gd_add_vol_op_versions_to_dict (dict_t *dict, char *prefix,
                                glusterd_volinfo_t *volinfo)
{
        int              ret      = -1;
        xlator_t        *this     = NULL;
        ...
        snprintf (key, sizeof (key), "%s.op-version", prefix);
        ...
        memset (key, 0, sizeof (key));
        snprintf (key, sizeof (key), "%s.client-op-version", prefix);
                             ...
                             char *prefix)
{
        ...
        memset (key, 0, sizeof (key));
        snprintf (key, sizeof (key), "%s%d", prefix, count);
        ret = gd_add_vol_op_versions_to_dict (dict, key, volinfo);
        if (ret)
                goto out;
 ...
 * or equal to GD_OP_VERSION_RHS_3_0, the op-version from which volume snapshot is
 * supported.
 */
int
...
/*
 * Imports the operating versions of a volume if required and available.
 *
 * RHS-2.1 doesn't export a volumes operating versions during a volume
 * sync, and instead calculates it during the import.
 */
int
gd_import_volume_op_versions (dict_t *dict, glusterd_volinfo_t *volinfo,
                              char *prefix, char *volname)
{
        int              ret           = -1;
        xlator_t        *this          = NULL;
        ...
        /* When working with RHS-2.1 and below, calculate the op-version from
         * the volume options instead of importing
         */
        if (conf->op_version < GD_OP_VERSION_RHS_3_0) {
        ...
        snprintf (key, sizeof (key), "%s.op-version", prefix);
        ret = dict_get_int32 (dict, key, &volinfo->op_version);
        if (ret) {
                gf_log (this->name, GF_LOG_ERROR, "%s missing in payload "
                        "for %s", key, volname);
                goto out;
        ...
        memset (key, 0, sizeof (key));
        snprintf (key, sizeof (key), "%s.client-op-version", prefix);
        ret = dict_get_int32 (dict, key, &volinfo->client_op_version);
        if (ret)
                gf_log (this->name, GF_LOG_ERROR, "%s missing in payload "
                        "for %s", key, volname);
out:
                         ...
                         char *prefix)
{
        ...
        ret = gd_import_volume_op_versions (peer_data, new_volinfo, key,
                                            volname);
        if (ret) {
                gf_log ("glusterd", GF_LOG_ERROR, "Failed to import op-versions"
...
gd_peerinfo_find_from_addrinfo (const struct addrinfo *addr)
{
        ...
        list_for_each_entry (peer, &conf->peers, uuid_list) {
                ret = getaddrinfo (peer->hostname, NULL, NULL,
                                   &paddr);
                if (ret) {
                        ...
                        gf_log (this->name, GF_LOG_TRACE,
                                ...
                                peer->hostname, gai_strerror (ret));
                        ret = 0;
                        continue;
                }

                for (candidate = paddr; candidate != NULL;
...
/* glusterd_friend searches for a peer which matches the
 * hostname @hoststr and if found sets @peer with the pointer
 * to the corresponding peerinfo object.
 * Returns -1 otherwise.
 *
                                  ...
                                  glusterd_peerinfo_t **peer)
{
        ...
        GF_VALIDATE_OR_GOTO (this->name, (peer != NULL), out);
        peerinfo = NULL;
