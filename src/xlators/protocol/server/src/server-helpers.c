...
get_frame_from_request (rpcsvc_request_t *req)
{
        ...
        this = req->trans->xl;
        priv = this->private;
        list_for_each_entry (trav, &priv->conns, list) {
                if (trav == conn) {
                        ...
                        */
                        if (!conn->username && req->pid != NFS_PID)
                ...
                while (username_str) {
                        if (!fnmatch (username_str, username, 0)) {
                                ...
                                ret = !((strcmp (data_to_str (passwd_data),
                                                    password))?0: -1);
                                if (!ret) {
                                        conn->username = gf_strdup (username);
                                        conn->passwd = gf_strdup (password);
                                }
                                if (ret == -1)
                                        gf_log ("auth/login", GF_LOG_ERROR,
