...
client_setvolume_cbk (struct rpc_req *req, struct iovec *iov, int count, void *myframe)
{
        ...
        */
        ...
        /* XXX: This is just a way we understand which version the
           server belongs to */
        ret = dict_get_str (reply, "server-pkg-version", &srv_pkgver);
        if (!ret) {
                gf_log (this->name, GF_LOG_INFO,
                        "'server-pkg-version' key not found, handshaked "
                        "with older client");
        }

        if (conf->server_version) {
                GF_FREE (conf->server_version);
                conf->server_version = NULL;
        }

        if (srv_pkgver)
                conf->server_version = gf_strdup (srv_pkgver);
