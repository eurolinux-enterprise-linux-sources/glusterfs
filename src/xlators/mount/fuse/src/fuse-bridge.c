...
fuse_inode_set_need_lookup (inode_t *inode, xlator_t *this)
{
	...
	if (priv->aux_gfid_mount) {
		...
		nodeid = (gf_fuse_nodeid_t *)value;
		if (nodeid)
			nodeid->need_lookup = 1;
	} else {
...
fuse_inode_needs_lookup (inode_t *inode, xlator_t *this)
{
	...
	if (priv->aux_gfid_mount) {
		...
		nodeid = (gf_fuse_nodeid_t *)value;
		if (nodeid) {
			need_lookup = nodeid->need_lookup;
			nodeid->need_lookup = 0;
			if (need_lookup)
				ret = _gf_true;
		}
	} else {
...
static int
fuse_gfid_newentry_chown_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
                              int32_t op_ret, int32_t op_errno,
                              ...
                              dict_t *xdata)
{
        ...
        if (op_ret) {
                gf_log ("fuse-aux-gfid-mount", GF_LOG_INFO,
                        "failed to set the ownership of the entry %s with "
                        "gfid (%s): %s",
                        state->loc.path, uuid_utoa (state->loc.gfid),
                        strerror (op_errno));
        }
                           ...
                           dict_t *xdata)
{
        ...
        if (op_ret) {
                gf_log ("fuse-aux-gfid-mount", GF_LOG_INFO,
                        "failed to create the entry %s with gfid (%s): %s",
                        state->loc.path, uuid_utoa (state->loc.gfid),
                        strerror (op_errno));
                goto err;
        }

        /* If the actual owner is "root", then no need to do a chown(),
         * saves time */
        if (!state->uid && !state->gid)
                goto err;

        tmp_stat.ia_uid = state->uid;
        tmp_stat.ia_gid = state->gid;

        STACK_WIND (frame, fuse_gfid_newentry_chown_cbk,
                    state->active_subvol,
                    state->active_subvol->fops->setattr,
                    &state->loc, &tmp_stat, (GF_SET_ATTR_UID | GF_SET_ATTR_GID),
                    NULL);
                       ...
                       struct iatt *postparent)
{
        ...
        if (op_ret) {
                gf_log ("fuse-aux-gfid-mount", GF_LOG_INFO,
                        "failed to heal the entry %s with gfid (%s): %s",
                        state->loc.path, uuid_utoa (state->loc.gfid),
                        strerror (op_errno));
        }
...
fuse_auxgfid_newfile_parse_args (xlator_t *this, data_t *data)
{
        ...
        if (blob_len < min_len) {
                gf_log ("fuse-aux-gfid-mount", GF_LOG_ERROR,
