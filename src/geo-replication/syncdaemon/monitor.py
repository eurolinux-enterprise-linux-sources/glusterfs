...
def distribute(*resources):
    ...
    def rotimes(mas, sla):
        r = len(mas) / len(sla)
        if (len(mas) % len(sla)) == 0:
            return r - 1
        else:
            return r
    sidx = range(len(slaves))
    fidx = list(sidx)
    for i in range(rotimes(mvol.bricks, slaves)):
        fidx.extend(rotate(sidx, -1))
    workerspex = [ (brick['dir'], slaves[idx]) for idx, brick in zip(fidx, mvol.bricks) if is_host_local(brick['host']) ]
